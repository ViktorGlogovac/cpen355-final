...
# Set confidence threshold (float between 0 and 1)
...
video_path = 'stpats.mov'  # Replace with your video path or use 0 for webcam
...
while True:
    ...
    if response.status_code == 200:
        ...
        for detection in detections:
            ...
            if class_name.lower() != 'person' or confidence < CONFIDENCE_THRESHOLD:
                continue  # Skip detections that are not persons or below confidence threshold
            ...
            # Put label with team and confidence
            label = f"{team_label}: {confidence * 100:.1f}%"
            cv2.putText(
                frame,
                label,
                (x1, y1 - 10),
    ...
    cv2.imshow('Team Detection', frame)
